...
public class DefaultCloudinarymediacoreService implements CloudinarymediacoreService
{
	private static final Logger LOG = LoggerFactory.getLogger(DefaultCloudinarymediacoreService.class);
	...
	protected final static String FIND_LOGO_QUERY = "SELECT {" + CatalogUnawareMediaModel.PK + "} FROM {"
			+ CatalogUnawareMediaModel._TYPECODE + "} WHERE {" + CatalogUnawareMediaModel.CODE + "}=?code";

	protected Optional<CatalogUnawareMediaModel> findExistingLogo(final String logoCode)
	{
	...
	{
		return DefaultCloudinarymediacoreService.class.getResourceAsStream("/cloudinarymediacore/sap-hybris-platform.png");
	}
