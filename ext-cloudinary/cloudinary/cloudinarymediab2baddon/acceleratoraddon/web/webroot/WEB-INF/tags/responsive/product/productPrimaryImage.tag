...
<c:choose>
    ...
    <c:otherwise>
        <c:set value="${ycommerce:productImage(product, '')}" var="primaryImage"/>
    </c:otherwise>
</c:choose>
	...
	<c:when test="${not empty primaryImage}">
		...
		<c:choose>
			<c:when test="${not empty primaryImage.altText}">
				...
				<c:otherwise>
				<img class="cld-responsive" data-src="${fn:escapeXml(primaryImageUrl)}" alt="${altTextHtml}" title="${altTextHtml}"/>
				</c:otherwise>
				</c:choose>
			</c:when>
			<c:otherwise>
			    <c:choose>
                    ...
                    <c:otherwise>
                        <img class="cld-responsive" data-src="${fn:escapeXml(primaryImageUrl)}" alt="${productNameHtml}" title="${productNameHtml}"/>
                    </c:otherwise>
                </c:choose>
			</c:otherwise>
		</c:choose>
