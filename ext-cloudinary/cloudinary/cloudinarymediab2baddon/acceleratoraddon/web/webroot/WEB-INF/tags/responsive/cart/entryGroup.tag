...
<%@ taglib prefix="cloudinaryCart" tagdir="/WEB-INF/tags/addons/cloudinarymediab2baddon/responsive/cart"%>
<%--
...
<c:choose>
    <c:when test="${not empty entryGroup.children}">
        <c:forEach items="${entryGroup.children}" var="group" varStatus="loop">
            <cloudinaryCart:entryGroup cartData="${cartData}" entryGroup="${group}"/>
        </c:forEach>
    </c:when>
    <c:otherwise>
        <tr>
                ...
                <table>
                    ...
                    <c:if test="${not empty entryGroup.orderEntries}">
                        <c:forEach items="${entryGroup.orderEntries}" var="entry">
                            <tr>
                                <td>
                                    <cloudinaryCart:cartItem cartData="${cartData}" entry="${entry}" index="${entryGroup.groupNumber}"/>
                                </td>
                            </tr>
                        </c:forEach>
                    </c:if>
