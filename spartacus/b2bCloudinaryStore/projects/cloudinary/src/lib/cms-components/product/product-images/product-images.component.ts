...
export class ProductImagesComponent {
  ...
  constructor(
    ...
    this.currentCloudinaryConfigService.list().subscribe((cloudinaryConfig) => {
      this.isCloudinaryGalleryEnabled =
        cloudinaryConfig.isCloudinaryGalleryEnabled;

      //this.CName = cloudinaryConfig.CName;
      if (this.isCloudinaryGalleryEnabled) {
        this.product$.subscribe((prod) => {
          this.loadGallerySourceCode().then(() => {
            this.renderProductGalleryWidget(
              cloudinaryConfig,
              prod.sapCCProductCode,
              prod.spinSetCode,
            );
          });
        });
  ...
  getProductCode(): any {
    return this.product$.subscribe((p) => {
      this.sapCCProductCode = p.sapCCProductCode;
      return p.sapCCProductCode;
    });
  }
